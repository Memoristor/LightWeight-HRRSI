...
class HybridTrainPipe(Pipeline):
    ...
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, mean, std, min_crop_size=0.08,
                 local_rank=0, world_size=1, prefetch_queue_depth=2, dali_cpu=False, shuffle=True, fp16=False):
        ...
        super(HybridTrainPipe, self).__init__(batch_size, num_threads, device_id,
                                              prefetch_queue_depth=prefetch_queue_depth, seed=-1)

        # Enabling read_ahead slowed down processing ~40%
        self.input = ops.FileReader(file_root=data_dir, shard_id=local_rank, num_shards=world_size,
                                    random_shuffle=shuffle, initial_fill=prefetch_queue_depth,
...
class HybridValPipe(Pipeline):
    ...
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, size, mean, std,
                 local_rank=0, world_size=1, prefetch_queue_depth=2, dali_cpu=False, shuffle=False, fp16=False):
...
class DaliIteratorCPU(DaliIterator):
    ...
    def __init__(self, fp16=False, mean=(0., 0., 0.), std=(1., 1., 1.), pin_memory=True, **kwargs):
        ...
        self.preproc_thread = threading.Thread(
            ...
            kwargs={'dali_iterator': self._dali_iterator, 'cuda_stream': self.stream, 'fp16': self.fp16,
                    'mean': self.mean, 'std': self.std, 'proc_next_input': self.proc_next_input,
                    'done_event': self.done_event, 'output_queue': self.output_queue, 'pin_memory': self.pin_memory})
