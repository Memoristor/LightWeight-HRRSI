...
__all__ = ['DeepLabV3plus', 'DeepLabV3plusResNet50', 'DeepLabV3plusResNet101']
...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_class):
        ...
        self.output = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
            nn.Conv2d(256, num_class, 1, stride=1),
        )
        initialize_weights(self)
...
class DeepLabV3plus(nn.Module):
    def __init__(self, num_class, in_channels=3, backbone='xception', pretrained=True,
                 output_stride=16, freeze_bn=False, **_):

        super(DeepLabV3plus, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, backbone=backbone, output_stride=output_stride,
...
class DeepLabV3plusResNet50(DeepLabV3plus):
    def __init__(self, num_class, in_channels=3, pretrained=True, output_stride=16, freeze_bn=False, **_):
...
class DeepLabV3plusResNet101(DeepLabV3plus):
    def __init__(self, num_class, in_channels=3, pretrained=True, output_stride=16, freeze_bn=False, **_):
        super(DeepLabV3plusResNet101, self).__init__(
            num_class=num_class,
            in_channels=in_channels,
            pretrained=pretrained,
            backbone='resnet101',
...
if __name__ == '__main__':
    ...
    model = DeepLabV3plusResNet101(num_class=6)
